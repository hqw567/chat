...
import { usePresetsStore } from '@/stores/modules/presets'
...
const { newConversationCounter, currentConversation, processingController }
  = storeToRefs(conversationsStore)
const { updateConversation } = conversationsStore
...
async function sendMessage(input: string, parentMessageId = null) {
  ...
  if (activePresetToUse.value && activePresetToUse.value.client === 'bing') {
    if (
  ...
  if (!stream) {
    const result = await fetch(`${VITE_API_BASE_URL}/conversation`, {
      ...opts,
          ...
          >
            ...
            <div class="text-xs text-white/50 mb-1">
              <template v-if="message.role === 'bot'">
                {{
                  activePresetToUse?.options?.clientOptions?.chatGptLabel
                    || 'AI'
                }}
              </template>
            ...
            <div
              ...
              v-html="
                message.role === 'user' || message.raw
                  ? parseMarkdown(message.text)
                  : parseMarkdown(message.text, true)
              "
            />
          </div>
        </div>
      ...
      >
        ...
        >
          <Transition name="fade" mode="out-in">
            ...
            <i-logos-bing
              v-else-if="
                activePresetNameToUse === 'bing'
                  || activePresetToUse?.client === 'bing'
              "
              class="w-10 h-10 p-2 block transition duration-300 ease-in-out rounded-lg hover:bg-black/30 cursor-pointer hover:shadow"
              :class="{
                'bg-black/30 shadow': isClientDropdownOpen,
        ...
        <button
          ...
          @click="
            sendMessage(regenerateData.input, regenerateData.parentMessageId)
          "
        >
          <i-pepicons-pop-refresh class="w-5 h-5" />
        </button>
        <button
          v-else
          :disabled="!!processingController"
        ...
        >
          <i-carbon-send-filled class="w-5 h-5" />
        </button>
      </div>
    </div>
  </div>
